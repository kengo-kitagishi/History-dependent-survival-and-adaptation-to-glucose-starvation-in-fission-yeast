...
• 反応の加速と促進（体積排除効果） 高濃度のクラウディング剤はエントロピー的に分子会合イベントを促進し、それによって分子反応を加速させるという基本的な化学的帰結をもたらします。この効果は、総排除体積の減少につながる反応、すなわち、溶液中の高分子複合体の形成、表面部位への結合、不溶性凝集体の形成、およびタンパク質のコンパクト化やフォールディングを促進します。
• タンパク質のフォールディングと安定性の維持 クラウディングはタンパク質のフォールディングを促進し、折りたたまれた状態（ネイティブ状態）のタンパク質を安定化させることが予測されています。
• 拡散制限反応への影響 多くの細胞機能、例えば遺伝子発現、生合成、代謝を駆動する反応は拡散制限であると考えられています。細胞質における高分子濃度（クラウディング）は、基質と酵素の近接効果と粘性の影響とのトレードオフを通じて、重要な生化学的反応の速度を最大化するように設定されていると推測されています。翻訳速度が通常の細胞質濃度でピークに達するという実験結果は、「最大速度仮説」と一致しており、クラウディングの最適な制御が効率的な機能に不可欠であることを示唆しています。
