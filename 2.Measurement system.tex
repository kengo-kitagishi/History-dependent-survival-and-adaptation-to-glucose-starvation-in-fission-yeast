...
干渉の各項を Fourier 空間で分けるために、物体光にオフアクシス（off-axis）の波数 $(k_m^{\mathrm{off-axis}}, k_n^{\mathrm{off-axis}})$ を与えてキャリア周波数を作る。$(m, n)$ は画素の離散座標である。本研究の共通光路系では、参照光は Fourier 面のピンホールで空間フィルタされて準平面波
\begin{equation}
U_r = A_r
\end{equation}
になり、物体光は Ronchi 格子で与えられたオフアクシスの波数を持つ:
\begin{equation}
...
\subsection{オフアクシスホログラムの形成}

オフアクシス DH では、物体光と参照光の干渉パターンをイメージセンサで記録する。試料の位置を $z = 0$ とすると、センサ上の物体光は
\begin{equation}
U_{s,m,n} = \frac{1}{M^2} A_s \exp[i\phi_{m,n}] \exp[-i(k_m^{\mathrm{off-axis}} m + k_n^{\mathrm{off-axis}} n)]
\end{equation}
である。$M$ は倍率、$A_s$ は物体光の振幅、$(k_m^{\mathrm{off-axis}}, k_n^{\mathrm{off-axis}})$ は Ronchi 格子が与えるオフアクシスのキャリア波数、$\phi_{m,n}$ は試料による位相遅れである。参照光は
\begin{equation}
U_{r,m,n} = A_r
\end{equation}
...
細胞の輪郭は Omnipose\cite{cutler2022omnipose}で得た。本研究の画像から手で注釈した約 3000 細胞でモデルを学習した。
