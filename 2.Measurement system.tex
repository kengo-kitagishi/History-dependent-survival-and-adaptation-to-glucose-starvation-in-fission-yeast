% NEXT: 2.9.1 位相感度の理論を日本語に起こす（plot 2.9 はある。paragraph は無い）。2.9.2・2.9.3 は Figure 3・2 が確定してから〔初期値〕
...
\subsection{位相感度の理論と実測}
% 旧「Phase sensitivity」節をここへ移した。式は 2.9 でしか使わない。
% 理論のあとに実測を続ける: 連続フレーム差の空間標準偏差/√2 を細胞のない領域で測り、shot-noise 限界と比べる。Figure 3A（測り直し要）。
...
\subsection{位置の精度}
% 参照グリッド登録の精度: 公称ステージ変位と実測変位の差、誤差ベクトル。Figure 3B（対応表を 32 bit 実数で作り直してから）。
