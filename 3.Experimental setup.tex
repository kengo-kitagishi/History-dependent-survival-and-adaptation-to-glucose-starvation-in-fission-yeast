...
\subsubsection{Defocus and numerical refocusing}

Small deviations from ideal 4f geometry (e.g., misalignment of the Fourier plane or image plane) introduce defocus aberrations. The defocus-induced phase error in the spatial frequency domain is given by:
\begin{equation}
\phi_{\mathrm{defocus}}(f_x, f_y) = -\pi \lambda (f_x^2 + f_y^2) \Delta z
\end{equation}
where $\Delta z$ is the axial displacement from the ideal focal position. 

Numerical refocusing can be performed by compensating for this phase error during hologram reconstruction using the angular spectrum propagation method (Section 3.4):
\begin{equation}
U(x,y,z_{\mathrm{focus}}) = \mathcal{F}^{-1}\left\{\tilde{U}(f_x, f_y) \cdot \exp\left[i\sqrt{k^2 - (2\pi f_x)^2 - (2\pi f_y)^2} \cdot \Delta z\right]\right\}
\end{equation}

For detailed mathematical derivations including the treatment of quadratic phase factors, ABCD matrix formalism, and point spread function analysis, see Appendix~\ref{appendix:4f_detailed}.
...
Glucose concentrations in EMM2 media were determined using the LabAssay™ Glucose kit (FUJIFILM Wako Pure Chemical Corporation, product code 291-94001) according to the manufacturer's protocol. Absorbance was measured at 505 nm.
