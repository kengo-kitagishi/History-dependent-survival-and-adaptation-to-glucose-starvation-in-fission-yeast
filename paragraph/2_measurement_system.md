...
- Implication: 各段階が小節に対応する。
- Bridge: 位置合わせ。

**P1 位置合わせ**
- Topic: 各時点の位相像をグリッド中心像に並進のみで合わせる。
- Evidence: ECC 最大化（Evangelidis 2008）、変換は並進のみ。計測中はその変位でステージを更新（2.7.6）。記録後は同じ方法で各フレームのグリッド中心からの変位を測る。コードは findTransformECC / MOTION_TRANSLATION。
...
- Implication: n_medium が式 eq:drymass に入る。直線性の結果は 2.9.6。
- Bridge: 細胞の輪郭。

**P4 セグメンテーション**
- Topic: 輪郭は Omnipose で得る。
- Evidence: 自前の画像から手で注釈した約 3,000 細胞で学習（Cutler 2022）。
- Implication: マスクが面積・輪郭・系譜の入力になる。
- Bridge: 系譜。

**P5 系譜追跡**
- Topic: マスクを面積でフレーム間に連結する。
- Evidence: 各チャネルの閉端に最も近い細胞が起点。隣接2マスクの面積和が前フレームの1細胞と合えば分裂、閉端側が親の識別子を継ぐ。整合しないフレームは失敗の印。系譜＝閉端を占め続ける細胞の連なり。
